...
export const layout = (title, author, id, isFavorite) => {
  return `
  <div class='books__item' id='${id}'>
        <h3>${author}</h3>
        <p>${title}</p>
          <div class='books__item-action-btns'>
                <button data-favorite='${id}' class="btn btn-favorite" id="btn-favorite" type="button">
                ...
                <button class="btn btn-trash" id='btn-trash' type="button">
        ...
        <button class="btn books-item__btn" data-modal>Read more</button>
        </div>
`;
};
